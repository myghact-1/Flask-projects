...
def index():
    ...
    return render_template('home.html', results=date_results, title='Food Tracker - Home')


# date is going to be 20170520
...
def view(date):
    ...
    log_cur = db.execute(
        '''select food.food_name, food.protein, food.carbohydrates, food.fat, food.calories 
        from log_date 
    ...
    return render_template('day.html', entry_date=date_result['entry_date'], pretty_date=pretty_date, food_results=food_results, log_results=log_results, totals=totals, title='Food Tracker - Daily')
...
def food():
    ...
    cur = db.execute(
        'select food_name, protein, carbohydrates, fat, calories from food')
    results = cur.fetchall()

    return render_template('add_food.html', results=results, title='Food Tracker - Food')
